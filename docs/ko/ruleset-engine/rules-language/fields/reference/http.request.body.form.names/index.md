---
title: http.request.body.form.names · Cloudflare 규칙 세트 엔진 docs
description: HTTP 요청의 양식 필드의 이름.
chatbotDeprioritize: false
source_url:
  html: >-
    https://developers.cloudflare.com/ruleset-engine/rules-language/fields/reference/http.request.body.form.names/
  md: >-
    https://developers.cloudflare.com/ruleset-engine/rules-language/fields/reference/http.request.body.form.names/index.md
...
---
# http\://www\.request.body.form.names 의 이메일 주소

`http.request.body.form.names`배열\<String>

HTTP 요청의 양식 필드의 이름.

때 Populated`Content-Type`헤더는`application/x-www-form-urlencoded`.

이름은 사전 처리되지 않으며 요청에 사용되는 원래 사례를 유지합니다. 그들은 요청에 따라 동일한 순서에 목록으로 만들어집니다.
