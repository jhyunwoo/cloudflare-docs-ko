---
title: http.request.body.form.values · Cloudflare 규칙 세트 엔진 docs
description: HTTP 요청의 양식 필드의 값.
chatbotDeprioritize: false
source_url:
  html: >-
    https://developers.cloudflare.com/ruleset-engine/rules-language/fields/reference/http.request.body.form.values/
  md: >-
    https://developers.cloudflare.com/ruleset-engine/rules-language/fields/reference/http.request.body.form.values/index.md
...
---
# http.request.body.form.values는

`http.request.body.form.values`배열\<String>

HTTP 요청의 양식 필드의 값.

때 Populated`Content-Type`헤더는`application/x-www-form-urlencoded`.

값은 사전 처리되지 않으며 요청에 사용되는 원래 사례를 유지합니다. 그들은 요청에 따라 동일한 순서에 목록으로 만들어집니다.
