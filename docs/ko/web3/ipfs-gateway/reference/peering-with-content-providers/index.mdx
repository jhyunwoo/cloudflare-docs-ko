...
공개 HTTP gateway처럼 많은 요청을 처리하는 IPFS node를 운영하고 있다면, 대량의 데이터를 제공하는 node들과 장기 연결을 유지함으로써 쿼리 속도를 높일 수 있습니다.

이 과정을 **Peering**이라고 하며, IPFS config 파일의 Peering 설정을 수정해 Cloudflare의 peer를 우선 사용하도록 IPFS에 지시할 수 있습니다.
