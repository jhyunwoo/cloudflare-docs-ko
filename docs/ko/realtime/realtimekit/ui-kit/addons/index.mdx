...
translation_source_html: 'https://developers.cloudflare.com/realtime/realtimekit/ui-kit/addons/'
translation_source_md: 'https://developers.cloudflare.com/realtime/realtimekit/ui-kit/addons/index.md'
...
RealtimeKit의 사전 제작 UI Kit 기능을 확장하는 UI Kit 애드온 컬렉션은 추가 대화형 부품과 향상된 미팅 경험을 위한 컨트롤을 갖추고 있습니다.

<RTKSDKSelector />

UI Kit addons 라이브러리는 다음과 같은 범주를 제공합니다.

#### 호스트 컨트롤

호스트 컨트롤은 참가자 권한을 관리하기 위해 호스트를 허용:

- **Camera Host 제어**- 제어 participant 카메라 권한
- **Mic 호스트 컨트롤**- Control participant 마이크 권한
- **채팅 호스트 컨트롤**- Control participant 채팅 권한

#### 모델 번호: Reactions

참가자를위한 대화 형 참여 기능 :

- **손 절상**- 그들이 말하는 신호에 손을 올리는 참가자를 허용
- **Reactions 관리자**- 회의 중에 emoji 반응 표시

#### 공급 업체

participant 타일 인터페이스를 사용자 정의:

- **부품 타일 메뉴**- participant 타일에 사용자 정의 메뉴 옵션 추가

#### 참가자 탭 액션

참가자 탭에 사용자 지정 작업을 추가:

- **참가자 메뉴 항목**- participant 작업에 사용자 지정 메뉴 항목을 추가
- **참가자 탭 액션**- 참가자 탭에 맞춤 작업 버튼을 추가
- **참여자 탭 토글**- 참가자 탭에 사용자 지정 toggle 컨트롤 추가

#### 영상 배경

participant 영상에 시각 효과를 적용하십시오:

- **영상 배경**- 비디오 스트림에 blur 또는 가상 배경 적용

#### 통제 막대기

회의 통제 막대기를 주문을 받아서 만드십시오:

- **사용자 정의 컨트롤 바 버튼**- 컨트롤 바에 사용자 정의 버튼을 추가
...
<RTKCodeSnippet id="web-react">
  ...
  :::참조
  자주 묻는 질문`RtkUiProvider`대신에`RtkMeeting`, 패스`meeting`·`config`공급자에 목표:
...
<RTKCodeSnippet id="web-web-components">
  ...
  이름 \*`rtk-meeting`HTML 파일에 있는 구성 요소:
  ...
  :::참조
  자주 묻는 질문`rtk-ui-provider`대신에`rtk-meeting`, 패스`meeting`·`config`공급자에 목표:
...
<RTKCodeSnippet id="web-angular">
  ...
  :::참조
  자주 묻는 질문`rtk-ui-provider`대신에`rtk-meeting`구성 요소, 패스`meeting`·`config`공급자에 목표:
...
<RTKCodeSnippet id="web-react">
  일부 addons 지원 programmatic 제어 동적 변경 동안 회의.

  #### 영상 배경

  적용, 교체, 또는 비디오 배경을 제거 할 수 있습니다.
