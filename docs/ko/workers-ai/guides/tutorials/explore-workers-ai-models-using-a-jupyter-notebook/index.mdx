...
title: Workers AI를 탐험하십시오 Jupyter 노트북을 사용하는 모델
tags:
  - AI
  - Python
head: []
description: >-
  이 Jupyter 노트북은 Python 및 요청 라이브러리를 사용하여 다양한 모델 (Whisper, Distilled BERT, LLaVA
  및 Meta Llama 3)를 탐색합니다.
...
translation_source_md: >-
  https://developers.cloudflare.com/workers-ai/guides/tutorials/explore-workers-ai-models-using-a-jupyter-notebook/index.md
...
당신은 할 수[Workers AI 노트북 다운로드](/workers-ai-notebooks/cloudflare-workers-ai.ipynb)아래의 임베디드 노트북을 볼 수 있습니다.

또는 당신은 이것을 실행할 수 있습니다[구글 코랩](https://colab.research.google.com/github/craigsdennis/notebooks-cloudflare-workers-ai/blob/main/cloudflare-workers-ai.ipynb)
...
## PythonXQXQXQXQXQXQ를 사용하여 Workers AIXQ API 탐색

[모델 번호: Workers AI](/workers-ai)Cloudflare 네트워크에서 기계 학습 모델을 실행할 수 있습니다, 자신의 코드에서 – Workers, Pages, 또는 REST API를 통해 어디에서든.

이 노트북은 Workers AI REST API를 사용하여 탐구합니다.[공식 Python SDK](https://github.com/cloudflare/cloudflare-python).
...
API를 사용하려면[모델 번호: Cloudflare 계정 ID](https://dash.cloudflare.com). AI에 머리 > Workers AI 페이지는 "사용 REST API". 이 페이지는 새로운 API 토큰을 만들고 계정 ID를 복사합니다.

환경 변수에 이러한 값을 추가하려면, 당신은 할 수 있습니다**새 파일 만들기**이름 \*`.env`이 노트북은 그 값을 읽을 것입니다.
...
그렇지 않으면 아래에서 신속하게 값을 입력 할 수 있습니다.
...
## Workers AI 플랫폼에서 작업 가능

### 텍스트 생성

모두 보기[텍스트 생성 모델](/workers-ai/models)
...
# **키보드 단축키를 사용하여 Jupyter Notebook에서 셀을 실행하십시오.**

Jupyter Notebook의 셀을 실행하면 다양한 키보드 단축키를 사용하여 신속하고 효율적으로 수행 할 수 있으며 시간과 노력을 절약 할 수 있습니다. 여기에 당신이 사용할 수있는 단축키입니다:

**한국어**

- **Shift + 입력**: 현재 셀을 실행하고 아래의 새로운 셀을 삽입합니다.
- **Ctrl + 입력**: 현재 셀을 실행하고 새로운 출력 디스플레이를 생성하지 않고 아래 새로운 셀을 삽입합니다.

**윈도우/리눅스**

- **Shift + 입력**: 현재 셀을 실행하고 아래의 새로운 셀을 삽입합니다.
- **Ctrl + 입력**: 현재 세포를 실행하고 다음 세포로 이동합니다.

**추가 단축키**

- **Alt + 입력**: 현재 셀을 실행하고 (Mac) 아래 새로운 출력 디스플레이를 만들거나 다음 셀 (Windows / Linux)로 이동하십시오.
- **Ctrl + Shift + 입력**: 현재 셀을 실행하고 (Mac) 아래 새로운 출력 디스플레이를 생성하거나 (Windows / Linux) 아래 새로운 세포를 만듭니다.

**팁과 트릭**

- 또한 사용할 수 있습니다**실행 셀**Jupyter Notebook 툴바의 버튼, 또는**지원하다**메뉴 옵션 (macOS) 또는**실행 -> 실행 셀**(윈도우 / 리눅스).
- 셀의 선택을 실행하려면, 사용**이동 + Alt + 입력**(Mac) 또는**Shift + Ctrl + 입력**(윈도우 / 리눅스).
- 세포를 실행하고 다음 세포로 이동하기 위해, 사용**Ctrl + Shift + 입력**(모든 플랫폼).

이 키보드 단축키를 사용해서, 당신은 당신의 Jupyter 노트북에서 능률적으로 그리고 빨리 일할 수 있을 것입니다. 해피 코딩!

### 본문 바로가기

모두 보기[이미지 모델에 텍스트](/workers-ai/models)
...
![파일 형식](/workers-ai-notebooks/cloudflare-workers-ai/assets/output_13_0.png)

### 본문 바로가기

모두 보기[본문 바로가기](/workers-ai/models/)모델 번호:
...
![lava 램프](https://blog.cloudflare.com/content/images/2017/11/lava-lamps.jpg)

이미지는 각종 착색한 lava 램프의 전시를 특색짓습니다. 장면에 적어도 14 lava 램프, 서로 다른 색상과 디자인. 램프는 시각적으로 호소하는 방식으로 배열되어 있으며, 전경과 다른 사람들에게 더 가까이 배치됩니다. 디스플레이는 눈길을 끄고 활기찬 분위기를 연출하며, 다양한 라바 램프를 제공합니다.

### 자동 음성 인식

모두 보기[음성 인식 모델](/workers-ai/models)
...
La inteligencia 인공 es bastante impresionante en estos días. Es un buen 순간 para ser un constructor

### 텍스트 분류

모두 보기[텍스트 분류 모델](/workers-ai/models)
...
모두 보기[이미지 분류 모형](/workers-ai/models#image-classification/)
...
![·](/workers-ai-notebooks/cloudflare-workers-ai/assets/output_27_0.jpg)
...
모두 보기[구매하기](/workers-ai/models#summarization)기반 모델
...
' 독립 선언은 7 월 4, 1776에 13 주에 의해 서명되었습니다. 미국 헌법의 첫 번째 시도였다. 정부를 변경하기 위해 사람들의 권리를 선언했습니다.
