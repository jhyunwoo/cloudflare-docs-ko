...
description: >-
  Meta's Llama 4 Scout는 네이티브 멀티모드 인 16 전문가가있는 17 억 매개 변수 모델입니다. 이 모델은 텍스트와 이미지
  이해에 업계를 선도하는 성능을 제공하는 혼합물의 구조체를 활용합니다.
chatbotDeprioritize: false
source_url:
  html: >-
    https://developers.cloudflare.com/workers-ai/models/llama-4-scout-17b-16e-instruct/
  md: >-
    https://developers.cloudflare.com/workers-ai/models/llama-4-scout-17b-16e-instruct/index.md
...
# llama-4-scout-17b-16e 구조

텍스트 생성 • 메타

@cf/meta/llama-4-scout-17b-16e 구조

Meta's Llama 4 Scout는 네이티브 멀티모드 인 16 전문가가있는 17 억 매개 변수 모델입니다. 이 모델은 텍스트와 이미지 이해에 업계를 선도하는 성능을 제공하는 혼합물의 구조체를 활용합니다.

| 모델 정보                                                                  |                                                                                     |
| ---------------------------------------------------------------------- | ----------------------------------------------------------------------------------- |
| Context 창[](https://developers.cloudflare.com/workers-ai/glossary/)    | 131,000 토큰                                                                          |
| 이용 약관                                                                  | [이름 \*](https://github.com/meta-llama/llama-models/blob/main/models/llama4/LICENSE) |
| 기능 호출[](https://developers.cloudflare.com/workers-ai/function-calling) | 이름 \*                                                                               |
| 공지사항                                                                   | 이름 \*                                                                               |
| 단위 가격                                                                  | M 입력 토큰 당 $0.27, M 출력 토큰 당 $0.85                                                    |

## 회사연혁

Workers AI LLM 놀이터와 함께이 모델을보십시오. 설정이나 인증이 필요 없으며 즉시 미리 볼 수 있으며 브라우저에서 모델을 직접 테스트 할 수 있습니다.

[LLM 놀이터 출시](https://playground.ai.cloudflare.com/?model=@cf/meta/llama-4-scout-17b-16e-instruct)
  ...
  - `messages`지원되는
    ...
    - `items`기타 제품
      ...
      - `tool_call_id`이름 \*
        ...
        도구 호출 ID. 000000001로 돌아갈 수 있는 것을 알고 있지 않다면
